...
const calculateMajorityValue = async () => {
  ...
  const majorityManualMapping = 
    manualMappingEnabledCount > manualMappingDisabledCount ? 'enabled' : 'disabled';

  // Calculate the majority for objectDisinfection
  const majorityObjectDisinfection = 
    objectDisinfectionEnabledCount > objectDisinfectionDisabledCount ? 'enabled' : 'disabled';
...
export const createUser = async (req, res) => {
  const { name, email, phoneNumber,IPAddress,organizationName, password, primaryContact, locations } = req.body;

   // Get the majority values for manualMapping and objectDisinfection
   const { majorityManualMapping, majorityObjectDisinfection } = await calculateMajorityValue();
...
export const getAllUserEmails = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "Failed to fetch user emails. Please try again." });
  }
};
...
export const getUserNameByEmail = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: 'Failed to fetch user name. Please try again.' });
  }
};
...
export const updateUserDetails = async (req, res) => {
  ...
  const updatedDetails = req.body; // New details to update
  ...
  try {
    ...
    // Update user details
    user.set(updatedDetails); // This updates all fields in updatedDetails
    await user.save();
...
export const getUserLocationNamesByEmail = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: "Failed to fetch location names. Please try again." });
  }
};
...
export const getSubLocationsByLocationName = async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.locations.length) {
      return res.status(404).json({ message: "Location not found for the specified user" });
    }

    // Extract sub-locations from the matched location
    const subLocations = user.locations[0].subLocations.map(subLocation => subLocation.name);

    return res.json({ subLocations });
  } catch (error) {
    console.error("Failed to fetch sub-locations:", error);
    return res.status(500).json({ message: "Failed to fetch sub-locations. Please try again." });
  }
};
